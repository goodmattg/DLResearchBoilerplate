...
class ChemModel(object):
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        accuracies = []
        accuracy_ops = [
            self.ops["accuracy_task%i" % task_id] for task_id in self.params["task_ids"]
        ]
        start_time = time.time()
        processed_graphs = 0
        batch_iterator = ThreadedIterator(
            self.make_minibatch_iterator(data, is_training), max_queue_size=5
        )
    ...
    def train(self):
        ...
        with self.graph.as_default():
            if self.args.get("--restore") is not None:
                _, valid_accs, _, _ = self.run_epoch(
            ...
            for epoch in range(1, self.params["num_epochs"] + 1):
                print("== Epoch %i" % epoch)
                train_loss, train_accs, train_errs, train_speed = self.run_epoch(
                    "epoch %i (training)" % epoch, self.train_data, True
                )
                accs_str = " ".join(
                    [
                        "%i:%.5f" % (id, acc)
                        for (id, acc) in zip(self.params["task_ids"], train_accs)
                    ]
                )
                errs_str = " ".join(
                    [
                        "%i:%.5f" % (id, err)
                        for (id, err) in zip(self.params["task_ids"], train_errs)
                    ]
                )
                print(
                    "\r\x1b[K Train: loss: %.5f | acc: %s | error_ratio: %s | instances/sec: %.2f"
                    % (train_loss, accs_str, errs_str, train_speed)
                )
                valid_loss, valid_accs, valid_errs, valid_speed = self.run_epoch(
                    "epoch %i (validation)" % epoch, self.valid_data, False
                )
                accs_str = " ".join(
                    [
                        "%i:%.5f" % (id, acc)
                        for (id, acc) in zip(self.params["task_ids"], valid_accs)
                    ]
                )
                errs_str = " ".join(
                    [
                        "%i:%.5f" % (id, err)
                        for (id, err) in zip(self.params["task_ids"], valid_errs)
                    ]
                )
                print(
                    "\r\x1b[K Valid: loss: %.5f | acc: %s | error_ratio: %s | instances/sec: %.2f"
                    % (valid_loss, accs_str, errs_str, valid_speed)
                )
                ...
                if val_acc < best_val_acc:
                    ...
                    print(
                        "  (Best epoch so far, cum. val. acc decreased to %.5f from %.5f. Saving to '%s')"
